...
- Selic/DI → custo de funding de FIDC (CDI+) → taxa mínima viável de originação → competitividade vs. bancos.
...
4. Feche com as implicações operacionais (crédito | alocação | long-short), cada uma em uma linha acionável.
...
2. Localize seu próprio arquivo: primeiro `.claude/agents/macro-economist.md` no projeto atual; se não existir, `~/.claude/agents/macro-economist.md`.
